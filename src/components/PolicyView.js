...
const { Shield, Map: MapIcon, Swords, Coins, Info } = window.LucideReact;

window.G1.components.PolicyView = ({ gameState, onUpdateAlignment }) => {
    const policies = [
        { id: 'safety', label: '安全重視', icon: Shield, desc: '冒険者の安全を第一に考えます。死亡率が低下しますが、戦力の成長は緩やかになります。', color: 'text-emerald-600' },
        { id: 'adventure', label: '冒険重視', icon: MapIcon, desc: '未知の探索を奨励します。新しいダンジョンの発見率と探索成功率が上がります。', color: 'text-indigo-600' },
        { id: 'military', label: '武力重視', icon: Swords, desc: '戦闘能力の向上に注力します。戦士系の能力が大幅に上がりますが、維持費が増加します。', color: 'text-rose-600' },
        { id: 'commerce', label: '商業重視', icon: Coins, desc: 'ギルドの収益を最優先します。酒場などの収入が増えますが、冒険者の忠誠度が下がりやすくなります。', color: 'text-amber-600' }
    ];
    ...
    const handleSliderChange = (id, value) => {
        ...
        // Simple redistribution of the difference to other policies
        ...
        if (currentTotal !== 100) {
            nextAlignment[otherIds[0]] += (100 - currentTotal);
        }
        ...
        <div key="policies" className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-500">
            <div className="flex justify-between items-center">
                ...
                <div className="flex items-center gap-2 bg-stone-100 px-3 py-1 rounded-full border border-stone-200">
                    ...
                    <span className="text-[10px] font-bold text-stone-500">合計配分: {total}%</span>
                </div>
            </div>
                ...
                {policies.map(policy => (
                    <div key={policy.id} className="bg-white border border-stone-200 p-5 rounded-sm shadow-sm relative overflow-hidden">
                        ...
                        <div className="flex justify-between mt-2">
                            <span className="text-[10px] font-bold text-stone-300">消極的</span>
                            <span className="text-[10px] font-bold text-stone-300">積極的</span>
                        </div>
                    </div>
                ))}
            </div>
