        ...
        <div key="home" className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-500 overflow-y-auto pr-1">
            ...
            {gameState.specialRequest && (
                <div className="bg-indigo-900 border-2 border-indigo-500 p-5 rounded-sm shadow-xl text-indigo-50 relative overflow-hidden shrink-0">
                    ...
                    <div className="relative z-10">
                        <h4 className="font-black text-2xl mb-2 flex items-center gap-2 text-indigo-200">
                            <Target className="w-8 h-8" /> 特殊指名依頼: {gameState.specialRequest.name}
                        </h4>
                        <p className="text-sm text-indigo-100 mb-4 leading-relaxed max-w-2xl">{gameState.specialRequest.desc}</p>
                        <div className="flex gap-6 text-sm font-bold mb-5 bg-indigo-950/50 p-3 rounded-sm border border-indigo-700/50">
                            <span>推奨戦力: <span className="text-rose-400 text-lg">{gameState.specialRequest.powerReq}</span></span>
                            <span>報奨金: <span className="text-amber-400 text-lg">{gameState.specialRequest.reward.toLocaleString()} G</span></span>
                        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-[#F2E8C6] border-2 border-[#D4C3A3] p-5 rounded-sm shadow-sm relative overflow-hidden group">
                    ...
                    <p className="text-stone-700 italic font-medium leading-relaxed relative z-10 pl-4">
                        「{gameState.currentRumor || '最近は特に変わった噂は聞かないな。'}」
                    </p>
                </div>
                {gameState.currentEvent && (
                    <div className="bg-rose-50 border-l-4 border-rose-500 p-4 rounded-sm shadow-sm flex items-start gap-4">
                        <AlertTriangle className="w-8 h-8 text-rose-600 shrink-0" />
                        <div>
                            <h4 className="font-black text-rose-900 text-sm mb-1">【世界情勢】{gameState.currentEvent.name}</h4>
                            <p className="text-xs text-rose-800 leading-relaxed">{gameState.currentEvent.desc}</p>
                        </div>
                    </div>
                )}
            </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                <div className="bg-white p-6 border-2 border-[#E8E0D5] rounded-sm shadow-sm flex flex-col">
                    ...
                    <div className="space-y-4 text-stone-700 font-medium flex-1">
                        <p className="leading-relaxed">
                            マスター、第 {currentYear} 暦 【{currentSeason}】の状況です。<br />
                            現在、金庫には <span className="font-bold text-amber-600 text-lg">{gameState.budget.toLocaleString()} G</span> の蓄えがあり、
                            <span className="font-bold text-stone-800"> {gameState.adventurers.length} 名</span> の冒険者が所属しています。
                        </p>
                        <div className="grid grid-cols-2 gap-4 pt-2">
                            <div className="bg-stone-50 p-3 rounded-sm border border-stone-100">
                                <div className="text-[10px] font-bold text-stone-400 uppercase">名声</div>
                                <div className="text-xl font-black text-indigo-700">{gameState.fame}</div>
                            </div>
                            <div className="bg-stone-50 p-3 rounded-sm border border-stone-100">
                                <div className="text-[10px] font-bold text-stone-400 uppercase">悪名</div>
                                <div className="text-xl font-black text-rose-800">{gameState.notoriety}</div>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-stone-800 p-6 border-2 border-stone-700 rounded-sm shadow-xl flex flex-col text-stone-200">
                    ...
                    <div className="space-y-4 flex-1">
                        <div className="flex justify-between items-center bg-stone-900/50 p-3 rounded-sm">
                            <span className="text-sm font-bold text-stone-400">予想総収入</span>
                            <span className="text-xl font-black text-emerald-400">+{forecast.income.toLocaleString()} G</span>
                        </div>
                        <div className="flex justify-between items-center bg-stone-900/50 p-3 rounded-sm">
                            <span className="text-sm font-bold text-stone-400">確定維持費</span>
                            <span className="text-xl font-black text-rose-400">-{forecast.expense.toLocaleString()} G</span>
                        </div>
                        <div className="pt-4 border-t border-stone-700 flex justify-between items-center">
                            <span className="font-black text-[#D9A94E]">次期純増減</span>
                            <span className={`text-3xl font-black ${forecast.net >= 0 ? 'text-emerald-500' : 'text-rose-500'}`}>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                <div className="bg-white border-2 border-[#D4C3A3] p-6 rounded-sm shadow-sm">
                    <h3 className="text-xl font-black text-stone-800 mb-4 flex items-center gap-3 border-b-2 border-stone-100 pb-3">
                        <HeartHandshake className="w-6 h-6 text-rose-600" /> 受付嬢・看板娘
                    </h3>
                    <div className="flex items-start gap-4">
                        <div className="w-24 h-24 bg-stone-100 rounded-sm flex items-center justify-center shrink-0 border-2 border-stone-200 overflow-hidden">
                            <img src="assets/images/hire_receptionist.png" className="w-full h-full object-cover" alt="" />
                        </div>
                        <div>
                            <div className="text-lg font-black text-indigo-900">{gameState.receptionist.name}</div>
                            <div className="text-[10px] font-bold text-stone-400 uppercase mb-2">種別: {gameState.receptionist.type}</div>
                            <p className="text-xs text-stone-600 leading-relaxed">{gameState.receptionist.desc}</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-white border-2 border-[#D4C3A3] p-6 rounded-sm shadow-sm">
                    <h3 className="text-xl font-black text-stone-800 mb-4 flex items-center gap-3 border-b-2 border-stone-100 pb-3">
                        <Swords className="w-6 h-6 text-rose-700" /> 他ギルドの動向
                    </h3>
                    <div className="space-y-3">
                        {gameState.rivals && gameState.rivals.slice(0, 3).map(rival => (
                            <div key={rival.id} className="flex justify-between items-center p-2 hover:bg-stone-50 rounded-sm transition-colors cursor-pointer border border-transparent hover:border-stone-100">
                                <div className="flex items-center gap-3">
                                    <div className={`w-2 h-2 rounded-full ${rival.relation >= 70 ? 'bg-emerald-500' : rival.relation >= 40 ? 'bg-amber-500' : 'bg-rose-500'}`}></div>
                                    <span className="font-bold text-sm text-stone-700">{rival.name}</span>
                                </div>
                                <div className="text-right">
                                    <span className="text-xs font-black text-stone-400">戦力 {rival.power}</span>
                                </div>
                            </div>
                        ))}
                    </div>
