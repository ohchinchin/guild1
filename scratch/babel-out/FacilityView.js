...
window.G1.components.FacilityView = function (_ref) {
    ...
    return React.createElement(
        'div',
        { key: 'facilities', className: 'space-y-4 animate-in fade-in slide-in-from-right-4 duration-500' },
        React.createElement(
            'p',
        ...
        React.createElement(
            ...
            facilities.map(function (fac) {
                var FacIcon = SafeIcon(fac.icon);
                var currentLevel = gameState.facilities[fac.id] || 1;
                var cost = 1000 + currentLevel * 500;
                var isMax = currentLevel >= 5;

                return React.createElement(
                    'div',
                    { key: fac.id, className: 'bg-white border-2 border-[#D4C3A3] p-4 rounded-sm shadow-sm flex flex-col hover:border-amber-400 transition-all group' },
                    React.createElement(
                        'div',
                        { className: 'flex items-center gap-3 mb-3 border-b border-stone-100 pb-3' },
                        React.createElement(
                            'div',
                            { className: 'bg-stone-100 p-2 rounded-sm text-stone-600 group-hover:bg-amber-50 group-hover:text-amber-700 transition-colors' },
                            React.createElement(FacIcon, { className: 'w-6 h-6' })
                        ),
                        React.createElement(
                            'div',
                            null,
